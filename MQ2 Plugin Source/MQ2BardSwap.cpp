...
// From http://www.macroquest2.com/phpBB2/viewtopic.php?t=9178
...
public:
	...
	{
		if (bSwapEnabled)
			strcpy_s(Destination, 5, "TRUE");
		else
			strcpy_s(Destination, 6, "FALSE");
		return true;
	}
...
PLUGIN_API VOID OnPulse(VOID)
{
		...
		for (int i = 0; SongSkills[i] != NULL; i++) {
			...
			if (!strcmp(szSkills[pCurrentSong->Skill], SongSkills[i])) {
				...
				if (InstrumentSettings[i].Enabled && !bExcluded) {
					SwapTo = i;
					DelayCounter = MQGetTickCount64() + ((bMeleeSwap ? MeleeDelay : 0) * 100);
				}
	...
	if (DelayCounter <= MQGetTickCount64() && SwapTo != iCurrentSwap) {
