...
//  https://www.macroquest2.com/phpBB3/viewtopic.php?p=166864#p166864
// Cr4zyb4rd patch: 4/10/2014
...
#define HUDTYPE_CTRLKEY      16
#define HUDTYPE_ALTKEY       32
#define HUDTYPE_SHIFTKEY     64
#define HUDTYPE_FLASHING    128

#define MAX_REPLICATED_STRING 512
...
VOID AddElement(PCHAR IniString)
{
	...
	*Index = 0;
	...
	if (!Index)
		return;
	if (tolower(IniString[0]) == 'r')
	{
		*Index = 0;
		IniString++;
		repeat = atoi(IniString);
		IniString = &Index[1];
	}
	...
	if (!Index)
		return;
	if (tolower(IniString[0]) == 'a')
	{
		*Index = 0;
		IniString++;
		Color.A = atoi(IniString);
		IniString = &Index[1];
	}
	...
	*pElement->Text = '\0';
	...
	{
		if ((strlen(pElement->Text) + strlen(IniString)) >= MAX_REPLICATED_STRING) break;
		strcat(pElement->Text, IniString);
	}
	pElement->Font = Font;
...
VOID HandleINI()
{
	...
	SkipParse = GetPrivateProfileInt(HUDSection, "SkipParse", 10, INIFileName);
...
PLUGIN_API VOID OnDrawHUD(VOID)
{
	...
	do
	{
		...
		{
			if (pElement->Type&HUDTYPE_CTRLKEY && !((PCXWNDMGR)pWndMgr)->KeyboardFlags[1]) continue;
			if (pElement->Type&HUDTYPE_ALTKEY && !((PCXWNDMGR)pWndMgr)->KeyboardFlags[2]) continue;
			if (pElement->Type&HUDTYPE_SHIFTKEY && !((PCXWNDMGR)pWndMgr)->KeyboardFlags[0]) continue;
			...
			{
				if (pElement->Type&HUDTYPE_FLASHING)
				{
					ARGBCOLOR Color;
					Color.A = 0xFF;
					Color.R = rand() % 256;
					...
					Color.G = rand() % 256;
					// pElement->Color = Color.ARGB;
					DrawHUDText(szBuffer, X, Y, Color.ARGB, pElement->Font);

				}
				else
					DrawHUDText(szBuffer, X, Y, pElement->Color, pElement->Font);
			}
		}
	} while (pElement = pElement->pNext);
}
