...
static void ClickSwitch(CVector3 pos, EQSwitch* pSwitch)
{
	...
	//we dont need the random thingy if its NULL eq will figure out how far away we are from the door etc.

	pSwitch->UseSwitch(GetCharInfo()->pSpawn->SpawnID, 0xFFFFFFFF, 0, NULL);
}
...
PLUGIN_API bool IsMeshLoaded()
{
	if(g_mq2Nav)
		return g_mq2Nav->Get<NavMesh>()->IsNavMeshLoaded();
	return false;
}
...
{
	if (m_pMoveToDoor || m_pMoveToItem)
	{
		//this is not optional since its one of these things that can make your character run into a wall or whatever forever ...
		//so always break out of this if the user wants control...
		m_pMoveToDoor = nullptr;
		m_pMoveToItem = nullptr;
	}
	if (m_isActive)
	{
...
bool MQ2NavigationPlugin::AttemptClick()
{
	if (!m_isActive)
		return false;
	...
	if (m_pEndingDoor)
	{
		float dist = pDisplay->TrueDistance(((PSPAWNINFO)pCharSpawn)->X, ((PSPAWNINFO)pCharSpawn)->Y, ((PSPAWNINFO)pCharSpawn)->Z, m_pEndingDoor->X, m_pEndingDoor->Y, m_pEndingDoor->Z, 0.0f);
		float BoundingRadius = 0;
		if (ActorBase*pBase = (ActorBase*)m_pEndingDoor->pSwitch) {
			BoundingRadius = pBase->GetBoundingRadius();
		}
		else {
			BoundingRadius = m_pEndingDoor->ScaleFactor * 0.01f;
		}
		if (dist <= (20.0f + ((PSPAWNINFO)pCharSpawn)->Height + BoundingRadius))
		{
			...
			//lets get closer
			//we are already looking at it, so we can just do a couple FORWARD moves
			m_pMoveToDoor = m_pEndingDoor;
			return false;
		}
	}
	else if (m_pEndingItem)
	{
		float dist = pDisplay->TrueDistance(((PSPAWNINFO)pCharSpawn)->X, ((PSPAWNINFO)pCharSpawn)->Y, ((PSPAWNINFO)pCharSpawn)->Z, m_pMoveToItem->X, m_pMoveToItem->Y, m_pMoveToItem->Z, 1.0f);
		if (dist <= 20.0f)
		{
...
void MQ2NavigationPlugin::CheckIfWeNeedTogetCloser()
{
	if (m_pMoveToDoor) {
		glm::vec3 dest(m_pMoveToDoor->X, m_pMoveToDoor->Y, m_pMoveToDoor->Z);
		LookAt(dest);
		float dist = pDisplay->TrueDistance(((PSPAWNINFO)pCharSpawn)->X, ((PSPAWNINFO)pCharSpawn)->Y, ((PSPAWNINFO)pCharSpawn)->Z, m_pMoveToDoor->X, m_pMoveToDoor->Y, m_pMoveToDoor->Z, 0.0f);
		float BoundingRadius = 0;
		if (ActorBase*pBase = (ActorBase*)m_pMoveToDoor->pSwitch) {
			BoundingRadius = pBase->GetBoundingRadius();
		}
		else {
			BoundingRadius = m_pMoveToDoor->ScaleFactor * 0.01f;
		}
		if (dist > (20.0f + ((PSPAWNINFO)pCharSpawn)->Height + BoundingRadius))
		{
			...
			MQ2Globals::ExecuteCmd(FindMappableCommand("FORWARD"), 0, 0);
			WriteChatf(PLUGIN_MSG "\agReached destination at: %.2f %.2f %.2f", dest.y, dest.x, dest.z);
			m_pMoveToDoor = nullptr;
		}
	} else if (m_pMoveToItem) {
		glm::vec3 dest(m_pMoveToItem->X, m_pMoveToItem->Y, m_pMoveToItem->Z);
		LookAt(dest);
		float dist = pDisplay->TrueDistance(((PSPAWNINFO)pCharSpawn)->X, ((PSPAWNINFO)pCharSpawn)->Y, ((PSPAWNINFO)pCharSpawn)->Z, m_pMoveToItem->X, m_pMoveToItem->Y, m_pMoveToItem->Z, 1.0f);
		if (dist > 20.0f) {
			MQ2Globals::ExecuteCmd(FindMappableCommand("FORWARD"), 1, 0);
